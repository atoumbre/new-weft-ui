<script lang="ts">
  ...
  const totalValue = $derived(items.reduce((sum: { add: (arg0: any) => any; }, item: { value: any; }) => sum.add(item.value), dec(0)))

  function findResource(resourceAddress: string) {
    const collateral = marketInfoStore.collateralResources.find(res => res.resourceAddress === resourceAddress)
    if (collateral)
      return collateral

    const loan = marketInfoStore.loanResources.find(res => res.resourceAddress === resourceAddress)
    if (loan)
      return loan

    const depositUnit = marketInfoStore.loanResources.find(res => res.lendingPoolState?.depositUnitAddress === resourceAddress)
    if (depositUnit)
      return depositUnit
    ...
    >
      ...
      <div class="space-y-3 max-h-96 overflow-y-auto">
        {#if items.length === 0 || totalValue.isZero()}
          <div class="text-center text-base-content/60 py-6">No data available</div>
        {:else}
          {#each items as { resourceAddress, amount, value } (resourceAddress)}
            ...
            <div class="p-3 bg-base-200/60 rounded-lg flex items-center justify-between gap-4">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-base-100 flex items-center justify-center overflow-hidden">
                  {#if iconUrl?.startsWith('http')}
                    <img src={iconUrl} alt={label} class="w-full h-full object-cover" />
                  {:else}
