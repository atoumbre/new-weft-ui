<script lang="ts">
  ...
  import HealthIndicator from '$lib/components/common/HealthIndicator.svelte';
  ...
  const ftWrappersByCdp: Record<string, FtWrapperPos[]> = {
    'CDP-001': [
      { id: 'ftw-1', wrapper: 'lpUSDC-DAI', shares: 1250.23456789, usd: 1250.23, underlyings: [ { logo: '💰', symbol: 'USDC' }, { logo: '🪙', symbol: 'DAI' } ] }
    ],
    'CDP-002': [
      { id: 'ftw-2', wrapper: 'stETH', shares: 2.3456789, usd: 5825.67, underlyings: [ { logo: '⟠', symbol: 'ETH' } ] }
    ],
    'CDP-003': []
  };
  ...
  const nftWrappersByCdp: Record<string, NftWrapperPos[]> = {
    'CDP-001': [ { id: 'nftw-1', collection: 'BlueChip Apes', count: 3, usd: 128_500, icons: ['🦍','🦍','🦍','🦍'] } ],
    'CDP-002': [ { id: 'nftw-2', collection: 'Art Blocks', count: 5, usd: 54_200, icons: ['🎨','🧩','🖼️','🖌️','🖼️'] } ],
    'CDP-003': []
  };
        ...
        <div class="flex-1 overflow-x-auto">
          <div class="flex items-center justify-end gap-2 w-max ml-auto">
            {#each cdpPositions as cdp}
              <button
                class={
                  "btn btn-sm " + (selectedCDP === cdp.id ? "btn-active btn-outline" : "btn-ghost")
                }
            ...
            {#if collatTab === 'normal'}
              <div class="mt-3 space-y-3">
                {#each currentCDP.collaterals as collateral, index}
                  <ListRow>
                    {#snippet left()}
                      <TokenCell logo={collateral.logo} symbol={collateral.asset} price={collateral.price} change={collateral.change24h} isPositive={collateral.isPositive} />
                    {/snippet}
                    {#snippet right()}
                      <div class="flex items-center gap-6">
                        <div class="text-right">
                          <AmountDisplay amount={parseUnitsStr(collateral.amount)} symbol={collateral.asset} usd={parseUsdStr(collateral.value)} />
                        </div>
                        <div class="text-right">
                        ...
                        <div class="flex -space-x-2">
                          {#each w.underlyings.slice(0, 4) as u}
                            <div class="avatar placeholder">
                              <div class="bg-base-200 w-6 h-6 rounded-full ring ring-base-100 ring-offset-1 text-[10px] flex items-center justify-center">{u.logo}</div>
                            </div>
                          {/each}
                          {#if w.underlyings.length > 4}
                            <div class="avatar placeholder">
                              <div class="bg-base-300 w-6 h-6 rounded-full ring ring-base-100 ring-offset-1 text-[10px] flex items-center justify-center">+{w.underlyings.length - 4}</div>
                            </div>
                          {/if}
                        </div>
                      </div>
                      ...
                      <div class="flex items-center gap-3">
                        ...
                        <div class="flex -space-x-2">
                          {#each n.icons.slice(0, 4) as ic}
                            <div class="avatar placeholder">
                              <div class="bg-base-200 w-6 h-6 rounded-full ring ring-base-100 ring-offset-1 text-[10px] flex items-center justify-center">{ic}</div>
                            </div>
                          {/each}
                          {#if n.icons.length > 4}
                            <div class="avatar placeholder">
                              <div class="bg-base-300 w-6 h-6 rounded-full ring ring-base-100 ring-offset-1 text-[10px] flex items-center justify-center">+{n.icons.length - 4}</div>
                            </div>
                          {/if}
                        </div>
                      </div>
          ...
          <div class="card-body py-3">
            ...
            <div class="mt-3 space-y-3">
              {#each currentCDP.loans as loan, index}
                <ListRow>
                  {#snippet left()}
                    <TokenCell logo={loan.logo} symbol={loan.asset} price={loan.price} change={loan.change24h} isPositive={loan.isPositive} />
                  {/snippet}
                  {#snippet right()}
                    <div class="flex items-center gap-6">
                      <div class="text-right">
                        <AmountDisplay amount={parseUnitsStr(loan.borrowed)} symbol={loan.asset} usd={parseUsdStr(loan.price) * parseUnitsStr(loan.borrowed)} />
                      </div>
                      <div class="text-right">
          ...
          <div class="card-body">
            ...
            <div class="mt-3 space-y-3">
              {#each availableLoanResources as loanResource}
                <ListRow>
                  {#snippet left()}
                    <TokenCell logo={loanResource.logo} symbol={loanResource.asset} price={loanResource.price} change={loanResource.change24h} isPositive={loanResource.isPositive} />
                  {/snippet}
                  {#snippet right()}
                    <div class="flex items-center gap-6">
                      <div class="text-right">
