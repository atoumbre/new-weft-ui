<script lang='ts'>
  ...
  function rowValid(r: Row): boolean {
    ...
    if ((r.type === 'add_collateral' || r.type === 'remove_collateral') && !collatSet.has(r.asset))
      return false
    if ((r.type === 'borrow' || r.type === 'repay') && !debtSet.has(r.asset))
      return false
    return true
  }
  ...
  function applyActions() {
    ...
    for (const a of actions) {
      if (a.type === 'add_collateral')
        nextColl[a.asset] = (nextColl[a.asset] ?? 0) + a.amount
      else if (a.type === 'remove_collateral')
        nextColl[a.asset] = Math.max(0, (nextColl[a.asset] ?? 0) - a.amount)
      else if (a.type === 'borrow')
        nextDebt[a.asset] = (nextDebt[a.asset] ?? 0) + a.amount
      else if (a.type === 'repay')
        nextDebt[a.asset] = Math.max(0, (nextDebt[a.asset] ?? 0) - a.amount)
    }
