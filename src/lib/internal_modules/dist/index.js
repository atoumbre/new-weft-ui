...
__export(index_exports, {
	...
	LENDING_MARKET_COLLATERAL_CONFIG_KVS: () => LENDING_MARKET_COLLATERAL_CONFIG_KVS,
	LENDING_MARKET_COLLATERAL_SERVICE_KVS: () => LENDING_MARKET_COLLATERAL_SERVICE_KVS,
	LENDING_MARKET_COMPONENT: () => LENDING_MARKET_COMPONENT,
	LENDING_MARKET_EFFICIENT_GROUP_KVS: () => LENDING_MARKET_EFFICIENT_GROUP_KVS,
	LENDING_MARKET_LOAN_CONFIG_KVS: () => LENDING_MARKET_LOAN_CONFIG_KVS,
	LENDING_MARKET_LOAN_SERVICE_KVS: () => LENDING_MARKET_LOAN_SERVICE_KVS,
	LENDING_MARKET_PRICE_CACHE_KVS: () => LENDING_MARKET_PRICE_CACHE_KVS,
	LENDING_MARKET_RES_CONFIG_KVS: () => LENDING_MARKET_RES_CONFIG_KVS,
	LENDING_POOL_COMPONENT: () => LENDING_POOL_COMPONENT,
	LENDING_POOL_INTEREST_STRATEGY_KVS: () => LENDING_POOL_INTEREST_STRATEGY_KVS,
	LENDING_POOL_RESOURCE_POOL_KVS: () => LENDING_POOL_RESOURCE_POOL_KVS,
	LENDING_POOL_SERVICE_KVS: () => LENDING_POOL_SERVICE_KVS,
	...
	PROTOCOL_INTEREST_SHARE: () => PROTOCOL_INTEREST_SHARE,
	ResourceDetailsRepo: () => ResourceDetailsRepo,
	STAKEHOLDER_REWARD_SHARE: () => STAKEHOLDER_REWARD_SHARE,
	WEFT_RESOURCE: () => WEFT_RESOURCE,
	WEFT_STAKING_COMPONENT: () => WEFT_STAKING_COMPONENT,
	WeftLedgerSateFetcher: () => WeftLedgerSateFetcher,
	XUSDC_RESOURCE: () => XUSDC_RESOURCE,
	ZERO: () => ZERO,
	dec: () => dec,
	defaultLendingPools: () => defaultLendingPools,
	duToResourceMapping: () => duToResourceMapping,
	findIndexes: () => findIndexes,
	optionTransformPlugin: () => optionTransformPlugin,
	resourceToDuMapping: () => resourceToDuMapping,
...
var FungibleResourceCollectionState = class {
	...
	getVaultState(vaultAddress) {
		return Object.values(this.values)
			.filter((v) => v.aggregationLevel === 'Vault')
	...
	async loadValues({ aggregationLevel, recursive, ledgerStateSelector }) {
		const result = await this.stateFetcher.stateApi.entityFungiblesPage({
			stateEntityFungiblesPageRequest: {
				at_ledger_state: this.cursor?.stateVersion
					? { state_version: this.cursor?.stateVersion }
					: ledgerStateSelector,
				address: this.entity,
...
var EntityStateFetcher = class _EntityStateFetcher {
	...
	addPlugins({ enumPlugins, tuplePlugins }) {
		if (enumPlugins) this.addEnumPlugin(enumPlugins);
		if (tuplePlugins) this.addTuplePlugin(tuplePlugins);
	}
	...
	async fetchEntityState(entityAddressesInput, localFetchOptions = this.options) {
					...
					(async () => {
						if (entity.fungible_resources) {
							v.$fungibleResources.setInitialValues({
								initialValues: {
									fungible_resources: entity.fungible_resources,
									state_version: result.ledger_state.state_version
								}
							});
					...
					(async () => {
						if (entity.non_fungible_resources) {
							v.$nonFungibleResources.setInitialValues({
								initialValues: {
									non_fungible_resources: entity.non_fungible_resources,
									state_version: result.ledger_state.state_version
								}
							});
		...
		if (fetchOptions.loadResourceDetails) {
			const resources = returnedStates
				.map((state) => {
					const resAddress = [];
			...
			for (let i = 0; i < returnedStates.length; i++) {
				Object.entries(returnedStates[i].$fungibleResources.values).forEach(([key, _]) => {
					...
					if (resState?.$type !== 'FungibleResource') return;
				...
				Object.entries(returnedStates[i].$nonFungibleResources.values).forEach(([key, _]) => {
					const resState = resourcesStates.find((r) => r.$entityAddress === key);
					if (resState?.$type !== 'NonFungibleResource') return;
	...
	async fetchNftData(nonFungibleResourceAddress, nonFungibleLocalIds, fetchOptions) {
		...
		while (nftIdCount > 0) {
			...
			nftIdCount -= nb;
			const idsToLoad = nonFungibleLocalIds
				.slice(nftIdCount, nftIdCount + nb)
			...
			this.apiCallCount++;
			if (!fetchResult?.non_fungible_ids) return [];
			...
			if (fetchOptions?.loadLocations ?? false) {
				...
				locations.non_fungible_ids.forEach((location) => {
					const indexes = findIndexes(
						returnedResult,
						(nftData) => nftData.$nonFungibleId === location.non_fungible_id
					);
					indexes.forEach((index) => {
						returnedResult[index].$owningVault = location.owning_vault_address;
						returnedResult[index].$owningVaultParent =
							location.owning_vault_parent_ancestor_address;
						returnedResult[index].$owningVaultAncestor =
							location.owning_vault_global_ancestor_address;
					});
				});
			}
		...
		switch (field.kind) {
			...
			case 'Enum': {
				const enumPlugin = this.enumPlugins.reverse().find((plugin) => {
					if (field.type_name === void 0 || field.type_name === null) return void 0;
					else return plugin.enumName.includes(field.type_name);
				});
				if (enumPlugin) {
			...
			case 'Tuple': {
				...
				field.fields.forEach((subField, index) => {
					const key = subField.field_name
						? toCamelCase(subField.field_name)
						: `_unnamedField${index}`;
					values[key] = this.fetchField(subField);
				});
				const tuplePlugin = this.tuplePlugins
					.reverse()
					.find((plugin) => plugin.tupleName === field.type_name);
...
var BaseModel = class {
	constructor(stateFetcher) {
		this.stateFetcher =
			stateFetcher ??
			new EntityStateFetcher(
				import_babylon_gateway_api_sdk.GatewayApiClient.initialize({
...
var EntityStateModel = class extends BaseModel {
	...
	async init() {
		if (!this.innerState.$entityAddress) return;
		this.innerState = (
			await this.stateFetcher.fetchEntityState([this.innerState.$entityAddress])
		)[0];
	}
	get state() {
...
var NonFungibleDataModel = class extends BaseModel {
	...
	async init() {
		if (!this.innerData.$entityAddress) return;
		this.innerData = (
			await this.stateFetcher.fetchNftData(this.innerData.$entityAddress, [
				this.innerData.$nonFungibleId
			])
		)[0];
	}
	get data() {
...
var ResourceDetailsRepo = class extends BaseModel {
	...
	async fetchResourceState(entityAddressesInput) {
		...
		if (addresses.length > 1) {
			const res = await this.stateFetcher.fetchResourceState(entityAddressesInput, {
				ledgerStateSelector: this.ledgerStateSelector
			});
...
// src/api.ts
var import_babylon_gateway_api_sdk2 = require('@radixdlt/babylon-gateway-api-sdk');
...
import_decimal2.default.set({
	precision: 50,
	rounding: import_decimal2.default.ROUND_DOWN,
	toExpNeg: -50,
	toExpPos: 50
});
var dec2 = (input) => new import_decimal2.default(input);
var ZERO2 = dec2(0);
var ONE2 = dec2(1);
	...
	fetchField(field) {
		...
		switch (field.kind) {
			...
			case 'Tuple': {
				...
				field.fields.forEach((subField, index) => {
					const key = subField.field_name
						? toCamelCase2(subField.field_name)
						: `_unnamedField${index}`;
					values[key] = this.fetchField(subField);
				});
				const tuplePlugin = this.tuplePlugins
...
var resourceToDuMapping = /* @__PURE__ */ new Map([
	[
		...
		{
			interestModel: '1',
			du: 'resource_rdx1th0gjs665xgm343j4jee7k8apu8l8pg9cf8x587qprszeeknu8wsxz'
		}
	],
	// XRD
	[
		...
		{
			interestModel: '1',
			du: 'resource_rdx1t4p82pms6r20k87rscms728tekujacd0sgxyysk7yvl0jgf56gvjuc'
		}
	],
	// LSULP
	[
		...
		{
			interestModel: '0',
			du: 'resource_rdx1thw2u4uss739j8cqumehgf5wyw26chcfu98newsu42zhln7wd050ee'
		}
	],
	// xUSDC
	[
		...
		{
			interestModel: '0',
			du: 'resource_rdx1t5ljp8amkf76mrn5txmmemkrmjwt5r0ajjnljvyunh27gm0n295dfn'
		}
	],
	// xUSDT
	[
		...
		{
			interestModel: '2',
			du: 'resource_rdx1thyes252jplxhu8qvfx6k3wkmlhy2f09nfqqefuj2a73l79e0af99t'
		}
	],
	// xWBTC
	[
		...
		{
			interestModel: '2',
			du: 'resource_rdx1t456hgpk6kwn4lqut5p2mqqmuuwngzhwxlgyyk9dwv4t5hmp37d7xf'
		}
	],
	// xETH
	[
		...
		{
			interestModel: '0',
			du: 'resource_rdx1t4kxe9n00hgzng02myj6a320qxcma2umxj8ygr795cc5m0hsj3p4l2'
		}
	],
	// hUSDC
	[
		...
		{
			interestModel: '0',
			du: 'resource_rdx1t48fy4e7d0zfzkky5yxvgaxvewp65ecv49vtccyawlulhegk3sw7kz'
		}
	],
	// hUSDT
	[
		...
		{
			interestModel: '2',
			du: 'resource_rdx1t4y98gg8r4mvlaf6tyut0natx874er06qy84ct3d5dvg0c3j2d6d4s'
		}
	],
	// hWBTC
	[
		...
		{
			interestModel: '2',
			du: 'resource_rdx1t5tcgsd0m6ptqsd0g70xu08tzdhy23ml5ql9xlmmv9wpchg3lw7dtk'
		}
	]
	// hETH
]);
...
var InterestStrategy = class {
	...
	getInterestRate(usage) {
		...
		for (let i = 0; i < len - 1; i++) {
			if (
				this.breakPoints[i].usage.toNumber() <= usage &&
				usage < this.breakPoints[i + 1].usage.toNumber()
			) {
				j = i;
				break;
			}
		...
		const interestRate =
			breakPoint.rate.toNumber() +
			(usage - breakPoint.usage.toNumber()) * breakPoint.slop.toNumber();
		return interestRate;
	}
};
...
// src/api.ts
import_decimal3.default.set({
	precision: 50,
	rounding: import_decimal3.default.ROUND_DOWN,
	toExpNeg: -50,
	toExpPos: 50
});
var WeftLedgerSateFetcher = class _WeftLedgerSateFetcher {
	...
	static setInstance(radixGatewayApi) {
		if (!this.instance) {
			const defaultApi =
				radixGatewayApi ??
				import_babylon_gateway_api_sdk2.GatewayApiClient.initialize({
					basePath: 'https://mainnet.radixdlt.com',
	...
	async getMarketInfos(ledgerStateSelector) {
		...
		const collateralConfigManagerEntries = new KeyValueStore2(
			LENDING_MARKET_COLLATERAL_CONFIG_KVS,
			this.stateFetcher
		);
		const loanConfigManagerEntries = new KeyValueStore2(
			LENDING_MARKET_LOAN_CONFIG_KVS,
			this.stateFetcher
		);
		const efficiencyGroupManagerEntries = new KeyValueStore2(
			LENDING_MARKET_EFFICIENT_GROUP_KVS,
			this.stateFetcher
		);
		const loanServiceManagerEntries = new KeyValueStore2(
			LENDING_MARKET_LOAN_SERVICE_KVS,
			this.stateFetcher
		);
		const collateralServiceManagerEntries = new KeyValueStore2(
			LENDING_MARKET_COLLATERAL_SERVICE_KVS,
			this.stateFetcher
		);
		const [lendingInfo, lendingPoolComponentStates, lendingMarketComponentStates] =
			await Promise.all([
		...
		if (!lendingMarketComponentState) throw new Error('Invalid State');
		const marketState = lendingMarketComponentState.$state;
		const nonFungibleResources = lendingMarketComponentState?.$nonFungibleResources;
		if (!marketState || !nonFungibleResources) throw new Error('Invalid State');
		const loanResources = [];
		...
		Object.entries(resourceConfigs.values).forEach(([address, config]) => {
			...
			if (config.loanConfig) {
				...
				loanResources.push({
					...
					riskConfig: loanConfigManagerEntries.getValue(`Current_${config.loanConfig.loanConfigId}`)
						.entry,
					services: loanServiceManagerEntries.getValue(`Resource_${address}`),
					lendingPoolState,
					resourceDetails:
						lendingPoolComponentState?.$fungibleResources.values[address]?.fungibleDetails,
					duResourceDetails:
						lendingPoolComponentState?.$fungibleResources.values[
							lendingPoolState.depositUnitAddress
						]?.fungibleDetails
				});
			}
			if (config.collateralConfig) {
				...
				config.collateralConfig.efficiencyGroupIds.forEach((p) => {
					...
					efficiencyConfigs[`${p}`] = {
						group: efficiencyConfig,
						config: collateralConfigManagerEntries.getValue(
							`Current_${efficiencyConfig.collateralConfigId}`
						).entry
					};
				});
				collateralResources.push({
					...
					riskConfig: collateralConfigManagerEntries.getValue(
						`Current_${config.collateralConfig.collateralConfigId}`
					).entry,
					efficiencyConfigs,
					services: collateralServiceManagerEntries.getValue(`Resource_${address}`),
					totalDeposit: dec2(0),
					// resourceInfo?.amount ?? dec(0),
					totalDepositDU: dec2(0),
					totalDepositUnderDU: dec2(0)
					// resourceDetails: resourceInfo?.fungibleDetails,
				});
			}
		});
		const lsuAmounts = [];
		Object.entries(lendingMarketComponentState?.$fungibleResources.values ?? {}).forEach(
			([address, data]) => {
				allFungibleResourceAddressesSet.add(address);
				const duIndex = loanResources.findIndex(
					(loanResource) => loanResource.lendingPoolState?.depositUnitAddress === address
				);
				...
				if (index > -1) {
					if (duIndex > -1) {
						...
						collateralResources[index].totalDepositUnderDU = dec2(
							duAmount.mul(loanResources[duIndex].lendingPoolState.depositUnitPrice)
						);
					} else {
						collateralResources[index].totalDeposit = collateralResources[index].totalDeposit.add(
							data.amount ?? dec2(0)
						);
						collateralResources[index].resourceDetails = data.fungibleDetails;
					}
				} else {
					if (!data.amount.eq(ZERO2)) {
						if (
							data.fungibleDetails?.$details.native_resource_details?.kind ===
								'ValidatorLiquidStakeUnit' &&
							!data.amount.eq(ZERO2)
						) {
							lsuAmounts.push({
								resourceAddress: address,
								amount: data.amount,
		...
		const resourceWithoutDetails = collateralResources
			.filter((resource) => !resource.resourceDetails)
			.map((resource) => resource.resourceAddress);
		const res = await this.getFetcher().fetchResourceState(resourceWithoutDetails);
		res.forEach((res2) => {
			if (res2.$type === 'FungibleResource') {
				const index = collateralResources.findIndex(
					(collateralResource) => collateralResource.resourceAddress === res2.$entityAddress
				);
				collateralResources[index].resourceDetails = res2;
			}
		});
	...
	async getMultipleCdp(ids, options = {}) {
		const result = await this.cdpRecursiveIndexer(ids, async (ids2) =>
			this.getCdpDataInternal(ids2, options)
		);
		return result;
	}
	...
	async getPrice(resources = defaultLendingPools, resourcesPerBatch = 10) {
		...
		for (let i = 0; i < resources.length; i += resourcesPerBatch) {
			batchedResources.push(
				resources.slice(i, i + resourcesPerBatch).map((resource) => `Address("${resource}")`)
			);
		}
		const prices = [];
		const manifests = batchedResources.map(
			(
				batchedResource
			) => `CALL_METHOD Address("${LENDING_MARKET_COMPONENT}") "get_price" Array<Address>(${batchedResource.join(',')});
`
		);
	...
	async getWeftStakingApr() {
		...
		if (!xusdcPrice) return emptyResult;
		...
		if (totalLendingDepositsUSD.eq(0)) return emptyResult;
		...
		if (!weftData || weftData.aggregation_level !== 'Global') return emptyResult;
		const stakedWeftAmount = dec2(weftData.amount);
		const weftPrice = priceMap.get(WEFT_RESOURCE);
		if (!weftPrice || stakedWeftAmount.eq(0)) return emptyResult;
		const stakedWeftValueUSD = stakedWeftAmount.mul(weftPrice);
		const stakingApr = averageLendingApr
			.mul(totalLendingDepositsUSD)
			.mul(PROTOCOL_INTEREST_SHARE)
			.mul(STAKEHOLDER_REWARD_SHARE)
			.div(stakedWeftValueUSD);
		return {
			apr: stakingApr.toNumber(),
			staked: stakedWeftAmount.toNumber(),
			tvl_xrd: stakedWeftValueUSD.toNumber(),
			tvl_usd: stakedWeftValueUSD.div(xusdcPrice).toNumber()
		};
	}
	/// PRIVATE METHODS
	async getCdpDataInternal(ids, options = {}) {
		...
		const manifests = batchedIds.map(
			(
				batchedId
			) => `CALL_METHOD Address("${LENDING_MARKET_COMPONENT}") "get_cdp" Array<NonFungibleLocalId>(${batchedId.join(',')});
`
		);
		const response = await Promise.all(
			manifests.map((manifest) =>
		...
		if (options.onProgress) options.onProgress(cdps.length);
		return cdps;
	}
    ...
    `;
		...
		for (const [res2, { state, config }] of Object.entries(result)) {
			const poolConfig = {
				...
				utilizationLimit:
					config.fields[4].variant_id === '0' ? void 0 : config.fields[4].fields[0].value,
				flashLoanAmountLimit:
					config.fields[5].variant_id === '0'
						? { variantName: 'None' }
		...
		encodedCDP[11].entries.forEach((loan) => {
			...
			loans[loan.key.value] = {
				...
				resourceConfig: {
					...
					efficiencyGroupId:
						loanData[7].fields[2].variant_id === '0'
							? void 0
			...
			collaterals[collateral.key.value] = {
				...
				configVersion: {
					...
					efficiencyMode: {
						...
						efficiencyGroupId:
							collateralData[7].fields[1].variant_id !== '1'
								? void 0
				...
				resourceConfig: {
					...
					efficiencyGroupIds: collateralData[8].fields[2].elements.map((e) =>
						Number.parseInt(e.value)
					)
				},
				isFromNft: false,
				resourceType:
					collateralData[10].variant_id === '0'
		...
		encodedCDP[13].entries.forEach((nftCollateral) => {
			...
			nft_entries.forEach((underlyingCollateral) => {
				...
				const nftValue = {
					...
					resourceType:
						nftCollateralData[1].fields[5].variant_id === '0'
