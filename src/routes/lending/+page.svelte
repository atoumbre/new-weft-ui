<script lang='ts'>
  ...
  const topPools = $derived(
    [...availablePools]
      .sort((a, b) => Number.parseFloat(b.supplyApr) - Number.parseFloat(a.supplyApr))
      .slice(0, 3),
  )
...
<div class='container mx-auto px-4 py-8 space-y-8'>
  <div>
    <h1 class='text-3xl font-bold mb-2'>Lending</h1>
    <p class='text-base-content/70'>Supply assets to earn yield and manage your lending positions</p>
  </div>
                ...
                <tr class='hover'>
                  ...
                  <td>
                    <div class='flex items-center gap-2'>
                      <progress class='progress progress-primary w-20' value={pool.utilization} max='100'></progress>
                      <span class='text-sm'>{pool.utilization}%</span>
                    </div>
                  </td>
                  <td>{pool.totalSupply}</td>
                  <td>
                    <button class='btn btn-sm btn-primary' onclick={() => openForm('supply', pool.asset)}>
                      Supply
                    </button>
                  </td>
                </tr>
