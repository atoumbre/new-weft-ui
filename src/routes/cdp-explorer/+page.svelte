<script lang='ts'>
  ...
  import { CDP_HEALTH_DEFINITIONS, getCdpHealthTextClass, resolveCdpHealthDefinition } from '$lib/cdp/health'
  ...
  const selectedResourceData = $derived.by((): ResourceAggregation | null => {
    if (!selectedLoanResource)
      return null
    return dropdownLoanResources.find(item => item.resourceAddress === selectedLoanResource) ?? null
  })
  ...
  const convertXrdToUsd = (xrdAmount: Decimal | undefined | null) => {
    if (!xrdAmount)
      return dec(0)
  ...
  const totalCollateralValue = $derived.by(() =>
    cdpStore.filteredCdpList.reduce((sum, cdp) => sum.add(convertXrdToUsd(cdp.totalCollateralValue)), dec(0)),
  )

  const totalDebtValue = $derived.by(() =>
    cdpStore.filteredCdpList.reduce((sum, cdp) => sum.add(convertXrdToUsd(cdp.totalLoanValue)), dec(0)),
  )

  const atRiskCdpCount = $derived.by(() =>
    cdpStore.filteredCdpList.filter((cdp) =>
      cdp.liquidationLtv.greaterThanOrEqualTo(dec(0.7 * 1.00)) && !inEfficiency(cdp),
    ).length,
  )
  ...
  const atRiskLoanAmount = $derived.by(() =>
    cdpStore.filteredCdpList
      .filter((cdp) => cdp.liquidationLtv.greaterThanOrEqualTo(dec(0.7 * 1.00)) && !inEfficiency(cdp))
  ...
  const collateralBreakdown = $derived.by((): ResourceAggregation[] => {
    ...
    return Array.from(breakdown.entries()).map(([resourceAddress, data]) => ({ resourceAddress, ...data }))
  ...
  const atRiskCdpHealthBreakdown = $derived.by(() => {
    const riskyCdps = cdpStore.filteredCdpList.filter(cdp =>
      cdp.liquidationLtv.greaterThanOrEqualTo(dec(0.7)) && !inEfficiency(cdp),
    )

    const total = riskyCdps.length || 0

    const buckets = CDP_HEALTH_DEFINITIONS
      .filter(definition => definition.id !== 'healthy')
...
<div class='container mx-auto px-4 py-8 space-y-6'>
  ...
  {#if cdpStore.loading}
    ...
    {@const progress = loadingState && loadingState.total > 0
      ? (loadingState.loaded / loadingState.total) * 100
      : 0}
  ...
  {#if !cdpStore.loading && cdpStore.filteredCdpList.length > 0}
    <FiltersPanel
      {showOnlyRiskyCdps}
      {sortByLoanResource}
      dropdownLoanResources={dropdownLoanResources}
