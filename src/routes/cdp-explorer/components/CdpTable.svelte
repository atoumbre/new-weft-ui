<script lang='ts'>
  ...
  function getNftCollateralInfo(cdp: CollateralizeDebtPositionData) {
    ...
    return nftPositions.reduce(
      (acc, nftPos) => {
        ...
        const totalValue = underlying.reduce(
          (sum, collateral) => sum.add(convertXrdToUsd(collateral?.value)),
          dec(0),
        )
        return {
  ...
  const tableData = $derived.by(() =>
    cdps.map((cdp: CollateralizeDebtPositionData) => {
            ...
            >
              ...
              <td class='text-right'>
                <div class='flex flex-col items-end'>
                  ...
                  >{fValue(collateralInfo.totalValue, { fullPrecision: false })}</span
                  >
                  <span class='text-xs text-base-content/60'
                  >{collateralInfo.count} position{collateralInfo.count !== 1 ? 's' : ''}</span
                  >
                </div>
              </td>
              ...
              <td class='text-right'>
                <div class='flex flex-col items-end'>
                  <span class='font-medium'
                  >{fValue(nftInfo.totalValue, { fullPrecision: false })}</span
                  >
                  <span class='text-xs text-base-content/60'
                  >{nftInfo.wrapperCount} wrapper{nftInfo.wrapperCount !== 1 ? 's' : ''}</span
                  >
                </div>
              </td>
            </tr>
