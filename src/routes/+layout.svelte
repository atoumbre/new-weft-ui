...
<div data-theme={theme} class='min-h-screen bg-base-100 text-base-content transition-colors'>
  <header
    class='sticky top-0 z-50 border-b border-base-200 bg-base-200/60 shadow-sm backdrop-blur supports-[backdrop-filter]:bg-base-200/40'
  >
    <div class='container mx-auto max-w-6xl px-6 py-3'>
      <div class='navbar min-h-0 p-0'>
        <div class='navbar-start'>
          <a
            href='/'
            class='flex items-center gap-2 no-underline transition-opacity hover:opacity-90'
          >
            <svg
            ...
            ><path d='M12 2l7 4v6c0 5-3.5 9.74-7 10-3.5-.26-7-5-7-10V6l7-4z' /></svg
            >
            <h1 class='text-xl font-semibold tracking-tight sm:text-2xl'>Weft Dashboard</h1>
            <div class='ml-2 badge badge-outline badge-sm badge-primary'>Beta</div>
          </a>
        </div>
        ...
        <div class='navbar-end gap-3'>
          ...
          <!-- Theme switcher -->
          <button
            class={`btn swap swap-rotate btn-ghost btn-sm ${theme === 'dark' ? 'swap-active' : ''}`}
            onclick={cycleTheme}
